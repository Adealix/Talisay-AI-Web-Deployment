...
const SITE_HEADER_H = 64; // matches Header.js HEADER_HEIGHT
...
export default function FloatingChatbot() {
  ...
  // Cap window height so it never reaches behind the site header on web
  const chatH = Platform.OS === 'web'
    ? Math.min(CHAT_H, screenH - SITE_HEADER_H - FAB_SIZE - 32)
    : CHAT_H;
  const msgListH = chatH - CHAT_HEADER_H - CHAT_INPUT_H;
  ...
  return (
    <View style={ST.container} pointerEvents="box-none">
      ...
      >
        ...
        <ScrollView
          ref={scrollRef}
          style={[{ height: msgListH }, Platform.OS === 'web' && { overflowY: 'auto' }]}
...
const ST = StyleSheet.create({
  ...
  input: {
    fontSize: 13,
    ...Platform.select({
      web: { outlineStyle: 'none', lineHeight: 18 },
      ios: { lineHeight: 18 },
      // Android: no lineHeight — it clips multiline text; paddingVertical: 0
      // removes the extra internal Android TextInput spacing
