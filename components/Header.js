...
const NAV_ITEMS = [
  ...
  { key: '/forum', label: 'Forum', icon: 'chatbubble-ellipses-outline', iconActive: 'chatbubble-ellipses' },
...
function NotifBell({ colors, onPress, count }) {
  ...
  return (
    <AnimatedPressable
      ...
      style={[animStyle, styles.bellBtn]}
    ...
    >
      <Ionicons
        ...
        size={22}
        color={count > 0 ? colors.primary : colors.textTertiary}
      />
      {count > 0 && (
          ...
          {isMobile ? (
            <>
              ...
              <Pressable onPress={() => router.push('/')} style={styles.mobileBrand}>
                <Text
                  style={[styles.mobileBrandText, { color: isDark ? '#e8f5e2' : '#1b4332' }]}
                  numberOfLines={2}
                >
                  {'Talisay\nOil'}
                </Text>
              ...
              <View style={styles.mobileRight}>
                <NotifBell colors={colors} count={unreadCount} onPress={() => router.push('/notifications')} />
                <ProfileDropdown />
                <ThemeToggle />
                <Pressable
