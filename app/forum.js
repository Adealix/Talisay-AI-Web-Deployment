...
import { checkForBadWords, getBadWordWarning } from '../utils/badWordsFilter';
import {
  View,
  Text,
  ScrollView,
...
// ─── Inline message banner (same style as Login page) ───
function InlineMessage({ text, type = 'error', onClose }) {
  const bg     = type === 'error'   ? '#fef2f2' : type === 'success' ? '#f0fdf4' : type === 'warning' ? '#fffbeb' : '#eff6ff';
  const border = type === 'error'   ? '#fecaca' : type === 'success' ? '#bbf7d0' : type === 'warning' ? '#fde68a' : '#bfdbfe';
  const color  = type === 'error'   ? '#dc2626' : type === 'success' ? '#16a34a' : type === 'warning' ? '#d97706' : '#2563eb';
  const icon   = type === 'error'   ? 'alert-circle' : type === 'success' ? 'checkmark-circle' : type === 'warning' ? 'warning' : 'information-circle';
  if (!text) return null;
  return (
    <Animated.View
      entering={FadeInDown.duration(250)}
      style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 8, backgroundColor: bg, borderWidth: 1, borderColor: border, borderRadius: 10, padding: 10, marginBottom: 12 }}
    >
...
function PostCard({ post, userId, onLike, onComment, onDelete, colors, isDark, index }) {
  ...
  const handleComment = async () => {
    ...
    try {
      const { hasBadWords } = checkForBadWords(commentText.trim());
      if (hasBadWords) {
        showToast(getBadWordWarning(), 'warning');
        return;
      }
    } catch (e) {
      console.warn('[forum] Bad words check error:', e);
    }
    ...
    try {
      ...
      console.error('[forum.handleComment]', e);
    } finally {
...
function CreatePostModal({ visible, onClose, onCreated, colors, isDark }) {
  ...
  const reset = () => { setTitle(''); setContent(''); setImages([]); setPostError(null); };
  ...
  const handleSubmit = async () => {
    ...
    // Bad words check (runs before showing loading state)
    try {
      const titleCheck = checkForBadWords(title.trim());
      const contentCheck = checkForBadWords(content.trim());
      if (titleCheck.hasBadWords || contentCheck.hasBadWords) {
        setPostError(getBadWordWarning());
        return;
      }
    } catch (e) {
      // Filter failure should not block posting
      console.warn('[forum] Bad words check error:', e);
    }
    ...
    try {
      ...
      if (result.ok && result.post) {
        ...
        const errorMsg = {
          title_and_content_required: 'Please fill in both the title and content.',
          server_error: 'Server error. Please try again.',
          network_error: 'Could not reach the server. Check your connection.',
          unauthorized: 'Your session has expired. Please log in again.',
        }[result.error] || result.error || 'Failed to create post. Please try again.';
        setPostError(errorMsg);
      }
    } catch (e) {
      console.error('[forum.handleSubmit]', e);
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose} statusBarTranslucent>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        {/* Outer container — flex centering */}
        <View style={styles.modalOverlay}>
          {/* Backdrop tap-to-close — absolutely behind the card */}
          <Pressable style={StyleSheet.absoluteFillObject} onPress={onClose} />
          {/* Modal card — sits above the backdrop via zIndex */}
          <View
          ...
          >
            ...
            >
              <InlineMessage text={postError} type="error" onClose={() => setPostError(null)} />

              <Text style={[styles.fieldLabel, { color: colors.textSecondary }]}>Title</Text>
              <TextInput
                value={title}
                onChangeText={t => { setTitle(t); setPostError(null); }}
              ...
              <TextInput
                value={content}
                onChangeText={t => { setContent(t); setPostError(null); }}
...
export default function ForumPage() {
  ...
  // Wait until auth is restored from storage before loading (avoids 401 on first render)
  useEffect(() => {
    if (!isInitializing) loadPosts(1);
  }, [loadPosts, isInitializing]);
  ...
  const handleComment = async (postId, text) => {
    const { hasBadWords } = checkForBadWords(text);
    if (hasBadWords) {
      Alert.alert('Inappropriate Language', getBadWordWarning());
      return false;
    }
