    ...
    >
      ...
      {!isUser && (
        <View style={[styles.avatar, {
          backgroundColor: isError ? '#ef444420' : isWarning ? '#f59e0b20' : '#22c55e20',
        }]}>
          <Ionicons
            name={isError ? 'alert-circle' : isWarning ? 'warning' : 'leaf'}
            size={16}
        ...
        style={[
          ...
          isUser
            ...
            : [styles.bubbleAI, {
                ...
                borderColor: isError ? '#ef444430' : isWarning ? '#f59e0b50' : colors.borderLight,
              }],
        ]}
      >
        {isUser ? (
...
export default function ChatbotPage() {
  ...
  const handleSend = useCallback(async (text) => {
    ...
    const { hasBadWords } = checkForBadWords(msgText);
    if (hasBadWords) {
      const now = formatTime();
      const warnMsg = {
        role: 'warning',
        text: '⚠️ Inappropriate language detected. Please keep conversations respectful and on-topic.',
        time: now,
      };
      setMessages(prev => [...prev, { role: 'user', text: msgText, time: now }, warnMsg]);
      setInput('');
      setHasStarted(true);
      scrollToBottom();
      return;
...
const styles = StyleSheet.create({
  ...
  input: {
    fontSize: 14,
    ...Platform.select({
      web: { outlineStyle: 'none', lineHeight: 20 },
      ios: { lineHeight: 20 },
      // Android: lineHeight on multiline TextInput clips text at top;
      // paddingVertical: 0 removes Android's built-in internal TextInput padding
