...
function transformMLResult(result) {
  ...
  if (result.multi_fruit || result.fruit_count != null) {
    ...
    const dominant = fruits.length > 0
      ...
      : null;
      // Guard rejection or no fruits survived — treat as non-Talisay so
      // the caller's isTalisay===false check fires and shows the guard message.
      const guardRejected = !!(result.user_message || result.error);
      const isTalisay = fruits.length > 0 && !guardRejected;
      return {
      ...
      // isTalisay is false when guard rejected or no fruits survived filtering
      isTalisay,
      userMessage: result.user_message || result.error || null,
      ...
      dimensionsConfidence: dominant?.detection_confidence || (isTalisay ? 0.85 : 0),
      ...
      yieldCategory: null,
      oilConfidence: null,
      overallConfidence: dominant?.detection_confidence || (isTalisay ? 0.85 : 0),
