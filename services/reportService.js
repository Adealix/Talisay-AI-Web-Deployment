...
export function generateCategoryCSV(category, historyItems, analytics) {
  ...
  if (isMulti) {
    ...
    csvContent += csvSummary(`A total of ${mfStats.count || total} batch scan(s) were performed, averaging ${fmt(mfStats.avgFruitCount)} fruits per batch. The average batch oil yield is ${fmt(mfStats.avgOilYield)}%, with yields ranging from ${fmt(mfStats.minOilYield)}% to ${fmt(mfStats.maxOilYield)}%. These batch analyses provide aggregated oil yield estimates across multiple fruits in a single scan.`);
  } else {
    csvContent += buildCSV(
      ['Metric', 'Value'],
      [
    ...
    csvContent += csvSummary(`The ${catLabel} category contains ${yieldStats.count || 0} sample(s) with a system average oil yield of ${fmt(yieldStats.avg)}%. ${sci ? `Research literature suggests an oil yield range of ${sci.oilYieldRange[0]}-${sci.oilYieldRange[1]}% (mean: ${sci.oilYieldMean}%) for this maturity stage.` : ''} Physical dimension estimates indicate average fruit length of ${fmt(dimStats.avgLength)} cm and average weight of ${fmt(dimStats.avgWeight)} g.`);
  }
  ...
  csvContent += csvSummary(`The ML model achieved an average confidence of ${pct(avgConf)} for ${catLabel} scans, with ${highConf} high-confidence (>=80%) and ${lowConf} low-confidence (<50%) predictions. ${highConf > lowConf ? 'The majority of predictions show strong model certainty.' : 'Model confidence varies across scans; further training data may improve accuracy.'}`);
  ...
  if (isMulti) {
    ...
    csvContent += csvSummary(`This table lists all ${items.length} individual batch scan records. Each record captures the number of fruits per batch, average oil yield, yield range, and model confidence. These detailed records enable per-batch analysis and comparison.`);
  } else {
    csvContent += buildCSV(
    ...
    csvContent += csvSummary(`This table lists all ${items.length} individual scan records for the ${catLabel} category. Each record includes model confidence scores, estimated oil yield, spot detection, and quality indicators. These records provide granular data for further analysis and research.`);
  }
...
export function generateOverallCSV(historyItems, analytics, users) {
  ...
  csv += csvSummary(`The system currently has ${ov.totalUsers || users.length} registered user(s) with a total of ${ov.totalHistory || historyItems.length} fruit scans processed. ${ov.scansThisWeek || 0} scan(s) were performed this week, indicating ${(ov.scansThisWeek || 0) > 5 ? 'active' : 'moderate'} system usage.`);
  ...
  {
    const greenPct = total > 0 ? (((catDist.GREEN || 0) / total) * 100).toFixed(1) : 0;
    const brownPct = total > 0 ? (((catDist.BROWN || 0) / total) * 100).toFixed(1) : 0;
    const dominant = (catDist.GREEN || 0) >= (catDist.YELLOW || 0) && (catDist.GREEN || 0) >= (catDist.BROWN || 0) ? 'Green / Immature' : (catDist.BROWN || 0) >= (catDist.YELLOW || 0) ? 'Brown / Overripe' : 'Yellow / Mature';
    csv += csvSummary(`The majority of scanned fruits fall under the ${dominant} category. Green (immature) fruits represent ${greenPct}% of all scans, while Brown (overripe) fruits account for ${brownPct}%. This distribution reflects the maturity stages of Talisay fruits captured by users in the system.`);
  }
  ...
  csv += csvSummary(`The overall average oil yield across all maturity categories is ${fmt(yieldOverall.avgYield)}%, ranging from ${fmt(yieldOverall.minYield)}% to ${fmt(yieldOverall.maxYield)}%. Brown (overripe) fruits generally exhibit the highest oil content, consistent with research literature indicating that oil yield increases with fruit maturity.`);
  ...
  csv += csvSummary(`The ML model achieved an average confidence of ${pct(conf.avgConfidence)} across all scans, with ${conf.highConfidenceCount || 0} high-confidence (>=80%) predictions and ${conf.lowConfidenceCount || 0} low-confidence (<50%) predictions. This indicates ${(conf.avgConfidence || 0) >= 0.75 ? 'strong' : 'moderate'} overall model reliability for fruit classification and oil yield estimation.`);
  ...
  csv += csvSummary(`Physical dimension estimates vary across maturity stages, with measurements derived from the coin-reference size estimation model. These dimensions help validate fruit identification and are consistent with the known botanical characteristics of Terminalia catappa fruits.`);
  ...
  {
    const coinRate = coinDet.totalScans > 0 ? ((coinDet.withCoin / coinDet.totalScans) * 100).toFixed(1) : 0;
    csv += csvSummary(`Coin reference detection was successful in ${coinRate}% of scans, enabling accurate physical size estimation. ${spots.withSpots || 0} scan(s) detected surface spots, which may indicate disease or environmental damage. Spot coverage analysis assists in assessing overall fruit quality.`);
  }
  ...
  csv += csvSummary(`There are ${users.length} registered user(s) in the system. User registrations reflect adoption of the Talisay AI system for oil yield analysis research and data collection.`);
  ...
  csv += csvSummary(`This table lists all ${historyItems.length} individual scan records in the system. Each record captures the fruit category, model confidence, estimated oil yield, and detection quality indicators. These records form the primary dataset for the Talisay AI oil yield analysis.`);
...
/**
 * Add an italic summary paragraph after a table in PDF.
 */
function addSummaryText(doc, text, y, pageWidth, pageHeight, logoBase64) {
  y = checkPage(doc, y, 18, pageHeight, pageWidth, logoBase64);
  doc.setFontSize(7.5);
  doc.setFont('helvetica', 'italic');
  ...
  doc.setDrawColor(27, 67, 50);
  doc.setLineWidth(0.3);
  const blockH = splitText.length * 3.5 + 4;
  doc.rect(15, y - 2, pageWidth - 30, blockH, 'F');
  doc.line(15, y - 2, 15, y - 2 + blockH);
  doc.text(splitText, 17, y + 1);
  y += blockH + 3;
  return y;
}
...
function htmlSummary(text) {
  return `<p style="font-style:italic;color:#444;font-size:11px;margin:4px 0 14px 0;padding:8px 10px;background:#f8fdf9;border-left:3px solid #1b4332;line-height:1.5;">${text}</p>`;
}
...
export async function generateCategoryPDF(category, historyItems, analytics) {
  ...
  if (category === 'MULTI_FRUIT') {
      ...
      const sections = [
        ...
        },
        {
          ...
          content: htmlKV([
            ...
            ['Average Confidence', pct(avgConf)],
          ]) + htmlSummary(`A total of ${mfStats.count || items.length} batch scan(s) were analyzed, averaging ${fmt(mfStats.avgFruitCount)} fruits per batch. The average batch oil yield is ${fmt(mfStats.avgOilYield)}%, with yields ranging from ${fmt(mfStats.minOilYield)}% to ${fmt(mfStats.maxOilYield)}%. These batch statistics provide insight into multi-fruit oil yield estimation performance.`),
        },
        {
          ...
          content: items.length > 0
            ? htmlTable(
              ...
              ) + htmlSummary(`This table lists all ${items.length} batch scan record(s) with individual batch details. Each row represents a single batch scan capturing multiple Talisay fruits, including the number of fruits detected, average oil yield, yield range, and model confidence score.`)
    ...
    y = addSummaryText(doc, `A total of ${mfStats.count || items.length} batch scan(s) were analyzed, averaging ${fmt(mfStats.avgFruitCount)} fruits per batch. The average batch oil yield is ${fmt(mfStats.avgOilYield)}%, ranging from ${fmt(mfStats.minOilYield)}% to ${fmt(mfStats.maxOilYield)}%. These batch statistics provide insight into multi-fruit oil yield estimation performance.`, y, pageWidth, pageHeight, logoBase64);
    ...
    if (items.length > 0) {
      ...
      y = addSummaryText(doc, `This table lists all ${items.length} batch scan record(s) with individual batch details. Each row represents a single batch scan capturing multiple Talisay fruits, including the number of fruits detected, average oil yield, yield range, and model confidence score.`, y, pageWidth, pageHeight, logoBase64);
    }
  ...
  if (Platform.OS !== 'web') {
    ...
    const sections = [
      ...
      },
      {
        ...
        ]) + htmlSummary(`The system recorded an average oil yield of ${fmt(yieldStats.avg)}% across ${yieldStats.count || 0} ${CAT_LABELS[category]} sample(s). Research literature indicates an expected range of ${sci.oilYieldRange[0]}-${sci.oilYieldRange[1]}% (mean: ${sci.oilYieldMean}%) for this maturity stage. ${Number(yieldStats.avg) >= sci.oilYieldRange[0] && Number(yieldStats.avg) <= sci.oilYieldRange[1] ? 'System results fall within the expected research range.' : 'System results may vary due to environmental factors and sample conditions.'}`),
      },
      {
        ...
        ]) + htmlSummary(`Physical dimensions for ${CAT_LABELS[category]} fruits show an average length of ${fmt(dimStats.avgLength)} cm and width of ${fmt(dimStats.avgWidth)} cm. The average whole fruit weight is ${fmt(dimStats.avgWeight)} g with a kernel weight of ${fmt(dimStats.avgKernelWeight)} g. These measurements are estimated using the coin-reference size detection model.`),
      },
      {
        ...
        content: htmlKV([
          ...
          ['Total with Confidence Data', confItems.length],
        ]) + htmlSummary(`The ML model achieved an average confidence of ${pct(avgConf)} for ${CAT_LABELS[category]} classifications. ${confItems.filter(h => h.confidence >= 0.8).length} scan(s) achieved high confidence (>=80%), indicating ${confItems.filter(h => h.confidence >= 0.8).length > confItems.filter(h => h.confidence < 0.5).length ? 'reliable model performance' : 'room for improvement'} in classifying this maturity stage.`),
      },
      {
        ...
        ]) + htmlSummary(`${spotsInCat.length} scan(s) detected surface spots, and ${coinInCat.length} scan(s) successfully detected the reference coin for size estimation. A coin detection rate of ${items.length > 0 ? ((coinInCat.length / items.length) * 100).toFixed(1) : 0}% indicates the frequency of accurate physical dimension measurement in this category.`),
      },
      {
        ...
        content: (items.length > 0
          ? htmlTable(
            ...
            ) + htmlSummary(`This table lists all ${items.length} individual scan records for ${CAT_LABELS[category]} fruits. Each record captures the model confidence, estimated oil yield percentage, yield category, and quality detection indicators. These records form the detailed dataset for this maturity category.`)
  ...
  y = addSummaryText(doc, `The system recorded an average oil yield of ${fmt(yieldStats.avg)}% across ${yieldStats.count || 0} ${CAT_LABELS[category]} sample(s). Research literature indicates an expected range of ${sci.oilYieldRange[0]}-${sci.oilYieldRange[1]}% (mean: ${sci.oilYieldMean}%) for this maturity stage. ${Number(yieldStats.avg) >= sci.oilYieldRange[0] && Number(yieldStats.avg) <= sci.oilYieldRange[1] ? 'System results fall within the expected research range.' : 'System results may vary due to environmental factors and sample conditions.'}`, y, pageWidth, pageHeight, logoBase64);
  ...
  y = addSummaryText(doc, `Physical dimensions for ${CAT_LABELS[category]} fruits show an average length of ${fmt(dimStats.avgLength)} cm and width of ${fmt(dimStats.avgWidth)} cm. The average whole fruit weight is ${fmt(dimStats.avgWeight)} g with a kernel weight of ${fmt(dimStats.avgKernelWeight)} g. These measurements are estimated using the coin-reference size detection model.`, y, pageWidth, pageHeight, logoBase64);
  ...
  y = addSummaryText(doc, `The ML model achieved an average confidence of ${pct(avgConf)} for ${CAT_LABELS[category]} classifications. ${confItems.filter(h => h.confidence >= 0.8).length} scan(s) achieved high confidence (>=80%), indicating ${confItems.filter(h => h.confidence >= 0.8).length > confItems.filter(h => h.confidence < 0.5).length ? 'reliable model performance' : 'room for improvement'} in classifying this maturity stage.`, y, pageWidth, pageHeight, logoBase64);
  ...
  y = addSummaryText(doc, `${spotsInCat.length} scan(s) detected surface spots, and ${coinInCat.length} scan(s) successfully detected the reference coin for size estimation. A coin detection rate of ${items.length > 0 ? ((coinInCat.length / items.length) * 100).toFixed(1) : 0}% indicates the frequency of accurate physical dimension measurement in this category.`, y, pageWidth, pageHeight, logoBase64);
  ...
  if (items.length > 0) {
    ...
    y = addSummaryText(doc, `This table lists all ${items.length} individual scan records for ${CAT_LABELS[category]} fruits. Each record captures the model confidence, estimated oil yield percentage, yield category, and quality detection indicators. These records form the detailed dataset for this maturity category.`, y, pageWidth, pageHeight, logoBase64);
  } else {
    doc.setFontSize(8);
...
export async function generateOverallPDF(historyItems, analytics, users) {
  ...
  if (Platform.OS !== 'web') {
    ...
    const sections = [
      ...
      },
      {
        ...
        ]) + htmlSummary(`The system currently has ${ov.totalUsers || users.length} registered user(s) with a total of ${totalScans} fruit scans processed. ${ov.scansThisWeek || 0} scan(s) were performed this week, indicating ${(ov.scansThisWeek || 0) > 5 ? 'active' : 'moderate'} system usage.`),
      },
      {
        ...
        content: htmlTable(
          ...
          const greenPct = catTotal > 0 ? (((catDist.GREEN || 0) / catTotal) * 100).toFixed(1) : 0;
          const brownPct = catTotal > 0 ? (((catDist.BROWN || 0) / catTotal) * 100).toFixed(1) : 0;
          const dominant = (catDist.GREEN || 0) >= (catDist.YELLOW || 0) && (catDist.GREEN || 0) >= (catDist.BROWN || 0) ? 'Green / Immature' : (catDist.BROWN || 0) >= (catDist.YELLOW || 0) ? 'Brown / Overripe' : 'Yellow / Mature';
          return htmlSummary(`The majority of scanned fruits fall under the ${dominant} category. Green (immature) fruits represent ${greenPct}% of all scans, while Brown (overripe) fruits account for ${brownPct}%. This distribution reflects the maturity stages of Talisay fruits captured by users in the system.`);
        })(),
      },
      {
        ...
        ]) + htmlSummary(`The overall average oil yield across all maturity categories is ${fmt(yieldOverall.avgYield)}%, ranging from ${fmt(yieldOverall.minYield)}% to ${fmt(yieldOverall.maxYield)}%. Brown (overripe) fruits generally exhibit the highest oil content, consistent with research literature indicating that oil yield increases with fruit maturity.`),
      },
      {
      ...
      },
      {
        ...
        ) + htmlSummary(`Physical dimension estimates vary across maturity stages, with measurements derived from the coin-reference size estimation model. These dimensions help validate fruit identification and are consistent with the known botanical characteristics of Terminalia catappa fruits.`),
      },
      {
        ...
        content: htmlKV([
          ...
          const coinRate = coinDet.totalScans > 0 ? ((coinDet.withCoin / coinDet.totalScans) * 100).toFixed(1) : 0;
          return htmlSummary(`Coin reference detection was successful in ${coinRate}% of scans, enabling accurate physical size estimation. ${spots.withSpots || 0} scan(s) detected surface spots, which may indicate disease or environmental damage. Spot coverage analysis assists in assessing overall fruit quality.`);
        })(),
      },
      {
        ...
        ) + htmlSummary(`There are ${users.length} registered user(s) in the system. User registrations reflect adoption of the Talisay AI system for oil yield analysis research and data collection.`),
      },
    ];
  ...
  y = addSummaryText(doc, `The system currently has ${ov.totalUsers || users.length} registered user(s) with a total of ${totalScans} fruit scans processed. ${ov.scansThisWeek || 0} scan(s) were performed this week, indicating ${(ov.scansThisWeek || 0) > 5 ? 'active' : 'moderate'} system usage.`, y, pageWidth, pageHeight, logoBase64);
  ...
  );
  {
    ...
    const brownPct = catTotal > 0 ? (((catDist.BROWN || 0) / catTotal) * 100).toFixed(1) : 0;
    const dominant = (catDist.GREEN || 0) >= (catDist.YELLOW || 0) && (catDist.GREEN || 0) >= (catDist.BROWN || 0) ? 'Green / Immature' : (catDist.BROWN || 0) >= (catDist.YELLOW || 0) ? 'Brown / Overripe' : 'Yellow / Mature';
    y = addSummaryText(doc, `The majority of scanned fruits fall under the ${dominant} category. Green (immature) fruits represent ${greenPct}% of all scans, while Brown (overripe) fruits account for ${brownPct}%. This distribution reflects the maturity stages of Talisay fruits captured by users in the system.`, y, pageWidth, pageHeight, logoBase64);
  }
  ...
  y = addSummaryText(doc, `The overall average oil yield across all maturity categories is ${fmt(yieldOverall.avgYield)}%, ranging from ${fmt(yieldOverall.minYield)}% to ${fmt(yieldOverall.maxYield)}%. Brown (overripe) fruits generally exhibit the highest oil content, consistent with research literature indicating that oil yield increases with fruit maturity.`, y, pageWidth, pageHeight, logoBase64);
  ...
  y = addSummaryText(doc, `The ML model achieved an average confidence of ${pct(conf.avgConfidence)} across all scans, with ${conf.highConfidenceCount || 0} high-confidence (>=80%) predictions and ${conf.lowConfidenceCount || 0} low-confidence (<50%) predictions. This indicates ${(conf.avgConfidence || 0) >= 0.75 ? 'strong' : 'moderate'} overall model reliability for fruit classification and oil yield estimation.`, y, pageWidth, pageHeight, logoBase64);
  ...
  y = addSummaryText(doc, `Physical dimension estimates vary across maturity stages, with measurements derived from the coin-reference size estimation model. These dimensions help validate fruit identification and are consistent with the known botanical characteristics of Terminalia catappa fruits.`, y, pageWidth, pageHeight, logoBase64);
  ...
  {
    const coinRate = coinDet.totalScans > 0 ? ((coinDet.withCoin / coinDet.totalScans) * 100).toFixed(1) : 0;
    y = addSummaryText(doc, `Coin reference detection was successful in ${coinRate}% of scans, enabling accurate physical size estimation. ${spots.withSpots || 0} scan(s) detected surface spots, which may indicate disease or environmental damage. Spot coverage analysis assists in assessing overall fruit quality.`, y, pageWidth, pageHeight, logoBase64);
  }
  ...
  y = addSummaryText(doc, `The Talisay kernel contains significant nutritional value per 100g, including essential fatty acids, proteins, and minerals. These nutritional properties contribute to the commercial and research interest in Talisay fruit oil extraction.`, y, pageWidth, pageHeight, logoBase64);
  ...
  y = addSummaryText(doc, `The fatty acid profile of Talisay kernel oil shows the composition of saturated and unsaturated fatty acids. The predominance of oleic and palmitic acids makes the oil suitable for various industrial and nutritional applications.`, y, pageWidth, pageHeight, logoBase64);
  ...
  y = addSummaryText(doc, `Seed-to-oil extraction data indicates a kernel oil content of ${SEED_TO_OIL.overallKernelOilContent}% with a kernel-to-fruit ratio of ${(SEED_TO_OIL.kernelToFruitRatio * 100).toFixed(0)}%. A typical tree yields approximately ${SEED_TO_OIL.typicalYieldPerTree.kgFruitPerYear} kg of fruit per year, translating to roughly ${SEED_TO_OIL.typicalYieldPerTree.litersOilPerYear} liters of oil annually.`, y, pageWidth, pageHeight, logoBase64);
  ...
  y = addSummaryText(doc, `There are ${users.length} registered user(s) in the system. User registrations reflect adoption of the Talisay AI system for oil yield analysis research and data collection.`, y, pageWidth, pageHeight, logoBase64);
  ...
  y = addSummaryText(doc, `The research references listed above provide the scientific foundation for oil yield ranges, nutritional data, and biological characteristics used in the Talisay AI analysis system. These peer-reviewed sources validate the accuracy of the system's estimations.`, y, pageWidth, pageHeight, logoBase64);
